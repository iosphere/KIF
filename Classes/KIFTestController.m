...
- (void)_advanceWithResult:(KIFTestStepResult)result error:(NSError *)error;
{
    ...
    switch (result) {
        ...
        case KIFTestStepResultSuccess: {
            [self _writeScreenshotForStep:self.currentStep withPrefix:[NSString stringWithFormat:@"%c-%.5d ", (char)[self.scenarios indexOfObject:self.currentScenario]+0x41,[self.currentScenario.steps indexOfObjectIdenticalTo:self.currentStep]]];
